...
class TeamController extends Controller
{
    ...
    public function accept_team(Request $request,$id){
        if(!empty($id) && $team_row=Team::where('id', $id)->get()->first()){
            if(!empty($request->user_id) && $user = User::where('id', $request->user_id)->first()){
                if($user->total_credits > 0){
                   
                    $team_row->status='accepted';
                    $team_row->update();
                    $user->total_credits = floatval($user->total_credits) - 1;
                    $user->update();
                    return response()->json(['msg' => 'Team Accepted','status'=>1], 200);
                }
                else{
                    return response()->json(['msg' => 'You have no credits in your account!','status'=>0], 200);
                }
            ...
            return response()->json(['msg' => 'Invalid team!','status'=>0], 200);
        }
    }
}
